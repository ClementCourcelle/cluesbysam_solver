
| NB  | "of the" | NB  | role      | pos               | be  | super_pos   |
| --- | -------- | --- | --------- | ----------------- | --- | ----------- |
| 1   | of the   | 3   | innocents | neighboring Alice | is  | in Column A |
| only 1 | of the | 3 | criminals | neighboring uma | is | to the right of vince |
| exactly 2 | of the | 3 | innocents | neighboring noah | are | above wanda |
| only 1 | of the | 3 | innocents | neighboring patrick | is | in row 5 |
| exactly 2 | of the | 3 | criminals | on the edges | are | tina's neighbors |
| only 1 | of the | 3 | criminals | in row 5 | is | wanda's neighbor |
| only 1 | of the | 6 | criminals | neighboring paula | is | to the right of ike |
| only 1 | of the | 3 | criminals | neighboring nicole | is | my neighbor |
| only 1 | of the | 2 | criminals | in row 3 | is | karen's neighbor |
| exactly 2 | of the | 5 | criminals | neighboring petra | are | in column c |
| exactly 2 | of the | 4 | criminals | in column c | are | linda's neighbors |
| exactly 2 | of the | 3 | innocents | in row 2 | are | paula's neighbors |
| only 1 | of the | 3 | innocents | in row 1 | is | chuck's neighbor |
| exactly 2 | of the | 6 | innocents | neighboring linda | are | in row 4 |
| exactly 2 | of the | 3 | innocents | on the edges | are | tina's neighbors |
| exactly 2 | of the | 5 | criminals | neighboring raul | are | in row 4 |
| only 1 | of the | 5 | criminals | neighboring isaac | is | above wanda |
| only 1 | of the | 3 | criminals | neighboring dana | is | above tina |
| only 1 | of the | 2 | criminals | neighboring avva | is | to the left of janet |
| exactly 2 | of the | 6 | criminals | neighboring ivan | are | in row 3 |
| only 1 | of the | 4 | criminals | neighboring gary | is | to the right of amy |
| only 1 | of the | 5 | criminals | neighboring hilda | is | in column b |
| exactly 2 | of the | 5 | criminals | on the edges | are | in column c |
| only 1 | of the | 2 | criminals | neighboring hope | is | above zane |
| only 1 | of the | 2 | criminals | neighboring mark | is | in column d |
| only 1 | of the | 2 | innocents | neighboring freya | is | brian's neighbor |
| only 1 | of the | 2 | criminals | neighboring olof | is | to the right of uma |
| only 1 | of the | 5 | criminals | on the edges | is | in row 2 |
| exactly 2 | of the | 4 | criminals | neighboring me | are | in between emily and xavi |
