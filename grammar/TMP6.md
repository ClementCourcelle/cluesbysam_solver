
| there   | "an" | PARITY | "number of" | role      | pos                   |
| ------- | ---- | ------ | ----------- | --------- | --------------------- |
| there's | an   | odd    | number of   | criminals | to the right of julie |
| there's | an   | odd    | number of   | criminals | on the edges          |
| there's | an   | odd    | number of   | criminals | below berat           |
| there's | an   | odd    | number of   | criminals | below alice           |
| there's | an   | odd    | number of   | innocents | below frank           |
| there's | an   | odd    | number of   | criminals | neighboring hope      |
| there's | an   | odd    | number of   | innocents | to the left of david  |
| there's | an   | odd    | number of   | criminals | neighboring hazel     |
| there's | an   | odd    | number of   | innocents | in column a           |
| there's | an   | odd    | number of   | innocents | neighboring olof      |
| there's | an   | odd    | number of   | innocents | below andre           |
| there's | an   | odd    | number of   | innocents | neighboring stella    |
| there's | an   | odd    | number of   | innocents | in row 1              |
| there's | an   | odd    | number of   | criminals | in the corners        |
| there's | an   | odd    | number of   | innocents | neighboring ronald    |
| there's | an   | odd    | number of   | criminals | neighboring janet     |
| there's | an   | odd    | number of   | innocents | neighboring tina      |
| there's | an   | odd    | number of   | innocents | neighboring nicole    |
| there's | an   | odd    | number of   | criminals | to the right of vince |
| there's | an   | odd    | number of   | innocents | in column b           |
| there's | an   | odd    | number of   | criminals | in column a           |
| there's | an   | odd    | number of   | innocents | neighboring john      |
| there's | an   | odd    | number of   | innocents | on the edges          |
| there's | an   | odd    | number of   | innocents | above uma             |
